...
class MoodAnalysisPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // 自動計算今天的日期與 13 天前 (包含今天共 14 天) 的日期
    final today = DateTime.now();
    final past = today.subtract(const Duration(days: 13));
    final dateRange = '${past.month.toString().padLeft(2, '0')}/${past.day.toString().padLeft(2, '0')} - ${today.month.toString().padLeft(2, '0')}/${today.day.toString().padLeft(2, '0')}';
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            // --- 標題區 (動態加入日期) ---
            Text(
              '近 14 日趨勢分析 ($dateRange)',
              style: const TextStyle(fontSize: 22.0, fontWeight: FontWeight.bold),
            ),
            const SizedBox(height: 24.0), // 原本的灰字已經刪除
